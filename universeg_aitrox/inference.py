...
if __name__ == '__main__':
    ...
    save_test_mha_path = os.path.join(test_set_dir , 'res_mha')
    for idx, item in enumerate(items):
        print(f"Processing body region {idx+1}/{len(items)}: {item}")
        support_folder = os.path.join(support_dir , item)
        test_folder =  os.path.join(test_set_dir , item + '_test/image')
        save_test_mask_path = os.path.join(test_set_dir , item + '_test/res_mask')
        save_test_vis_path = os.path.join(test_set_dir , item + '_test/res_vis')
        os.makedirs(save_test_mask_path, exist_ok=True)
        os.makedirs(save_test_vis_path, exist_ok=True)
        os.makedirs(save_test_mha_path,exist_ok=True)
        s_data = load_folder(support_folder, size)
        ...
        t_data = load_test_folder(test_folder, size)
        item_images = {}
        ...
        for i, img in enumerate(t_data.data):
            ...
            mask_mha = np.where(mask_np == 255, 1, mask_np)
            mask_mha = np.squeeze(mask_mha)
            assert len(mask_mha.shape)==2
            if t_data.names[i].split('_')[0] not in item_images.keys():
                item_images[t_data.names[i].split('_')[0]]=[sitk.GetImageFromArray(np.zeros_like(mask_mha))] * int(t_data.names[i].split('_')[3].split('.')[0])
            item_images[t_data.names[i].split('_')[0]][int(t_data.names[i].split('_')[2])]=sitk.GetImageFromArray(mask_mha)

        print(f"Finished processing body region {idx+1}/{len(items)}: {item}")

        for patient_id in item_images.keys():
            mha_save_folder = os.path.join(save_test_mha_path,patient_id)
            os.makedirs(mha_save_folder,exist_ok=True)
            sitk.WriteImage(sitk.JoinSeries(item_images[patient_id]),  os.path.join(mha_save_folder,f"{patient_id}_{item}.mha"),useCompression=True)
